...
export default function OutboundPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5">
      <h1 className="text-3xl font-black text-white tracking-tight leading-none">출고 관리</h1>
...
function InstructTab({ onDone }) {
  ...
  if (orders.length === 0)
    return <p className="text-center text-slate-500 py-12">대기 중인 출고오더가 없습니다.</p>
...
function CompleteTab({ onDone }) {
  ...
  if (loading) return <p className="text-center text-slate-400 py-12 animate-pulse">불러오는 중...</p>
  if (orders.length === 0)
    return <p className="text-center text-slate-500 py-12">출고완료 대기 중인 오더가 없습니다.</p>
